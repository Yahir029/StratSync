...
CREATE TABLE public.auditoria_codigos_acceso (
    ...
    CONSTRAINT auditoria_codigos_acceso_accion_check CHECK (((accion)::text = ANY ((ARRAY['CREADO'::character varying, 'MODIFICADO'::character varying, 'ELIMINADO'::character varying])::text[])))
);
...
CREATE TABLE public.profesores (
    ...
    fecha_creacion timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    creado_por integer,
    CONSTRAINT profesores_anos_experiencia_check CHECK ((anos_experiencia >= 0))
);
...
COPY public.profesores (id, nombres, apellidos, correo, telefono, fecha_nacimiento, especialidad, anos_experiencia, biografia, categoria_id, foto_perfil, codigo_acceso_maestro, activo, fecha_creacion, creado_por) FROM stdin;
\.
...
SELECT pg_catalog.setval('public.auditoria_codigos_acceso_id_seq', 1, false);
...
ALTER TABLE ONLY public.auditoria_codigos_acceso
    ADD CONSTRAINT auditoria_codigos_acceso_profesor_id_fkey FOREIGN KEY (profesor_id) REFERENCES public.profesores(id);
